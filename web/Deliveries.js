...
function orders() {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        if (row.transaction.transactionID != id) {
            if (row.transaction.trasnactionStatus == "READY" || row.transaction.trasnactionStatus == "IN TRANSIT") {
...
function getWeights() {
    ...
    for (let i = 0; i < orderAllArray.length; i++) {
        if (orderAllArray[i].transaction.originalLocationID == "DIEP" || orderAllArray[i].transaction.originalLocationID == "OROM") {
